...
all_days_soup = bs(all_days_page.content, 'lxml')
...
try:
    for all_votes_link in all_days_soup.find('tbody').findAll('a'):
        ...
        all_votes_soup = bs(requests.get(base_link + all_votes_link['href']).content, 'lxml')
        ...
        for row in all_votes_soup.find("tbody").findAll('tr'):
            ...
            vote_soup = bs(requests.get(base_link + vote_link['href']).content, 'lxml')
            try:
            ...
            for cell in list(all_cells):
                ...
                party_results_page = bs(requests.get(base_link + party_results_link["href"]).content, 'lxml')
                try:
                ...
                for i in range(int(len(all_results_cells)/2)):
                    ...
                    dictionary_data = {'date': date_formatted
